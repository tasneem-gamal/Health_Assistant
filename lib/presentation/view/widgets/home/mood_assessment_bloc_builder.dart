...
class MoodAssessmentBlocBuilder extends StatefulWidget {
  final InMemoryChatController chatController;
  final VoidCallback onFinish;
  final MentalHealthChatCubit mentalHealthChatCubit;
  final String? sessionId;

  const MoodAssessmentBlocBuilder({super.key, required this.chatController, required this.onFinish, required this.mentalHealthChatCubit, this.sessionId});

  @override
  State<MoodAssessmentBlocBuilder> createState() => _MoodAssessmentBlocBuilderState();
}

class _MoodAssessmentBlocBuilderState extends State<MoodAssessmentBlocBuilder> {
  int currentIndex = 0;
    ...
    return BlocBuilder<AssessmentCubit, AssessmentState>(
      builder: (context, state) {
        if (state is AssessmentLoading) {
          ...
          final stressAssessments = state.assessments
              .where((e) =>
                  e.assessmentTitle.toLowerCase().contains("stress assessment"))
              .toList();

          if (stressAssessments.isEmpty) {
            return const Center(child: Text("Stress Assessment not found"));
          }

          if (moodAssessment == null) {
            moodAssessment = stressAssessments.first;
